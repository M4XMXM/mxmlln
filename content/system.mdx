...
The fixed signature mark, top-left. Two Lottie faces: the **signature** (`Sig2026.json`) on index pages, and the **spin** (`LogoSpin2026.json`) on article pages, which spins with scroll direction. It uses `mix-blend-mode: difference` + `invert(1)` so it stays legible over any background it scrolls past. Reference: `app/blog/BlogLogo.tsx`, `app/system/layout.tsx`.
...
The labeled popover list — white card, rounded, soft shadow. Top-level items sit flush; nested items indent and shrink; the item in view gets a bold, full-contrast label. This is the reusable primitive; the Minimap (a fixed right-rail of notches that reveals this menu on hover and tracks scroll via `IntersectionObserver`) is a construct built on top of it. Reference: the popover in `app/blog/Minimap.tsx`.
