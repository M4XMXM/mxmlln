...
The signature mark, fixed top-left. Two faces — a static **signature** and a **spin** — both vector (Lottie). Drawn with `mix-blend-mode: difference` so it stays legible over whatever sits behind it.
...
Floating glass pill, centered up top. Uppercase Archivo links at low opacity; the active link gets a solid white fill.
